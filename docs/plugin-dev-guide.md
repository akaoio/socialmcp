...
The transport is an **HTTP long-poll relay** on `localhost:8765`:
...
Tool calls will **timeout** (default 30 s, 90 s for `post`) if the extension is not loaded and connected. No external dependencies — uses Node `http` built-in and the service worker's native `fetch`.
